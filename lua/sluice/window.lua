...
function M.new(i, gutter_settings)
  local window = {
    index = i,
    bufnr = nil,
    ns_id = nil,
  ...
  window.bufnr = M.vim.api.nvim_create_buf(false, true)
  window.ns_id = M.vim.api.nvim_create_namespace('sluice' .. window.bufnr)
  window.win_id = M.vim.api.nvim_open_win(window.bufnr, false, {
    relative = 'win',
    width = gutter_width,
    height = height,
    row = 0,
  ...
  local function update_window_size(ctx)
    ...
    height = M.vim.api.nvim_win_get_height(0)
    -- in case the window size changed, we can keep up with it.
    M.vim.api.nvim_win_set_config(window.win_id, {
  ...
  function window:set_gutter_lines(lines, count_method, width)
    ...
    M.vim.api.nvim_buf_set_lines(window.bufnr, 0, win_height - 1, false, strings)
  end

  --- Add styling to the gutter.
  function window:refresh_highlights(lines)
    M.vim.api.nvim_buf_clear_namespace(window.bufnr, window.ns_id, 0, -1)
    for i2, matches in ipairs(lines) do
      ...
      if best_texthl ~= nil then
        ...
        if best_linehl ~= nil then
          ...
          M.vim.api.nvim_buf_add_highlight(window.bufnr, window.ns_id, highlight_name, i2 - 1, 0, -1)
        else
          M.vim.api.nvim_buf_add_highlight(window.bufnr, window.ns_id, best_texthl, i2 - 1, 0, -1)
        end
      else
        M.vim.api.nvim_buf_add_highlight(window.bufnr, window.ns_id, best_linehl, i2 - 1, 0, -1)
      end
    end
  end

  M.au_id = M.vim.api.nvim_create_autocmd({"WinResized" }, {
