...
describe('line_to_gutter_line()', function()
  it('returns the same line number for line mode', function()
    assert.are.equal(convert.line_to_gutter_line(1, 100, 50), 1)
    assert.are.equal(convert.line_to_gutter_line(50, 100, 50), 50)
    assert.are.equal(convert.line_to_gutter_line(100, 100, 50), 100)
  end)
end)

describe('line_to_gutter_line_macro()', function()
  it('converts file lines to gutter lines correctly', function()
    assert.are.equal(convert.line_to_gutter_line_macro(1, 100, 50), 1)
    assert.are.equal(convert.line_to_gutter_line_macro(50, 100, 50), 25)
    assert.are.equal(convert.line_to_gutter_line_macro(100, 100, 50), 50)
  end)

  it('handles edge cases', function()
    assert.are.equal(convert.line_to_gutter_line_macro(1, 1000, 10), 1)
    assert.are.equal(convert.line_to_gutter_line_macro(1000, 1000, 10), 10)
  end)
end)
